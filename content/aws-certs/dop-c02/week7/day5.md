...
A) Terraform으로 이전 — HCL로 전면 재작성하되 기존 CFN StackSets/Drift 거버넌스 자산은 모두 폐기하고 tfstate 잠금 인프라(S3+DynamoDB)를 새로 구축
B) SAM 또는 CDK로 이전 — 둘 다 Apache 2.0 OSS, CFN 거버넌스(StackSets, Drift) 유지
C) v3에 영구히 고정 — 업그레이드 비용은 피하지만 보안 패치·신규 런타임 지원 중단으로 장기 운영 위험 누적
D) 자체 IaC 도구를 사내 개발 — 라이선스 비용은 없지만 합성·배포·롤백 엔진 전체를 직접 유지보수해야 함
...
A) Provisioned Concurrency 100개 상시 유지 — cold start는 0이 되지만 활성 시간 과금이 24시간 발생해 비용 최소화 요건에 위배
B) SnapStart 활성화 + Runtime Hook(`afterRestore`)에서 SecureRandom 재초기화 → 300~500ms로 단축, 비용 무료
C) Java를 Node.js로 전면 재작성 — init은 빨라지지만 Spring 생태계 포기 + 수개월 리팩토링 + 회귀 위험을 떠안음
D) Reserved Concurrency 100 설정 — 동시 실행 한도만 보장할 뿐 init 8초 자체는 그대로라 P99 latency 개선 없음
...
A) Spark on EMR 클러스터를 상시 운영 — 처리량은 충분하지만 클러스터 프로비저닝·튜닝·패치의 운영 부담이 크고 일배치에 과한 고정 비용
...
C) Lambda를 객체당 5000만 번 직접 호출 — 동시성 한도·throttle·부분 실패 추적·재시도 오케스트레이션을 전부 직접 구현해야 함
D) Glue Crawler로 스키마 발견 후 처리 — Crawler는 카탈로그 메타데이터 생성 도구라 5000만 객체의 변환·집계 작업에는 부적합
...
A) Lambda 핸들러 코드의 로직 버그 — 로컬에서 모든 통합 테스트를 통과했다면 동일 코드가 프로덕션에서만 즉시 throttle되는 현상은 코드 버그로 설명되지 않음
B) VPC 내부 RDS Proxy 연결 시 ENI 생성에 8~10초 소요. 로컬은 호스트 네트워크라 이 latency가 에뮬레이션되지 않음
C) 실행 IAM Role 부재 — 권한이 없으면 AccessDenied로 즉시 실패하지 throttle 후 30초 timeout으로 이어지지 않음
D) API Gateway 통합 설정 오류 — 매핑이 틀렸다면 로컬 sam local invoke 단계에서 먼저 드러났을 가능성이 높음
...
A) 각 리전 CFN Deploy Action 3개 + Manual Approval로 알람 발동 시 운영자가 콘솔에서 USE1·EUW1을 수동 역순 롤백 — 사람 개입이 끼어 RTO를 위반
B) Step Functions Action + ASL로 각 리전 ResultPath에 deploymentId 누적 + Choice로 알람 체크 + Catch로 역순 RollbackEUW1 → RollbackUSE1 자동
C) Lambda 하나에 3리전 배포·알람 체크·역순 롤백 로직을 모두 인라인 구현 — 분기·재시도·역순 보상 트랜잭션이 코드로 엉켜 디버깅·관측이 불가능
D) 리전별 독립 Pipeline 3개로 분리 — 각 파이프라인이 서로를 모르므로 리전 간 순차 의존성과 EUW1 실패 시 USE1 역순 롤백을 표현할 수 없음
...
A) CodePipeline ManualApprovalAction 3개로 보안팀·DevOps·배포 단계를 직렬 구성 — 승인은 되지만 7일 자동 취소 타이머와 Slack interactive message 인증 콜백 흐름을 별도로 구현해야 함
B) Standard Step Functions + `.waitForTaskToken` 3회 + SNS→Slack→Lambda(SendTaskSuccess) + TimeoutSeconds: 604800
C) Express Step Functions + StartSyncExecution으로 3단 승인 처리 — Express는 TaskToken 미지원이고 최대 실행 5분이라 7일 대기 자체가 불가능
D) EventBridge Scheduler로 승인 단계를 cron 트리거 — 정기 실행 도구라 사람 승인 대기·콜백·역순 분기를 표현할 수 없음
...
B) Lambda 함수 코드 자체의 런타임 버그가 hotswap 변경을 무효화 — 코드 버그는 정규 배포가 변경을 덮어쓰는 현상과 인과관계가 없음
C) sync 대상과 deploy 대상의 Region 불일치로 다른 함수에 적용 — 동일 프로파일·스택을 쓰는 워크플로에서 리전이 어긋날 구조적 이유가 없음
D) 여러 Lambda Layer 버전 간 충돌로 코드가 롤백 — Layer 버전 충돌은 배포 실패를 낼 뿐 hotswap 변경을 조용히 되돌리지 않음
...
A) 모든 팀이 SAM Policy Template만 공유해 사용 — IAM 정책 보일러플레이트는 줄지만 Lambda+Dynamo+API+X-Ray 패턴 전체를 추상화하는 재사용 단위가 없어 반복 작성이 그대로 남음
B) 사내 L3 Construct 라이브러리 작성 — 표준 패턴(보안·태깅·로깅·tracing)을 추상화한 `StandardApiFunction` 같은 Construct 배포
C) CloudFormation YAML 템플릿을 공유 폴더에 두고 복사 사용 — 복붙 기반이라 패턴이 드리프트되고 표준 변경 시 1500곳을 일일이 수정해야 함
D) 1500개 함수 코드를 단일 monorepo로 통합 — 코드 위치만 모을 뿐 추상화가 없고 팀 간 결합도만 증가시킴
...
A) selfMutation을 영구히 비활성화 — 깨진 파이프라인 사고는 막지만 GitOps "파이프라인도 코드" 자동 갱신의 핵심 가치를 통째로 포기
B) Break-glass IAM 권한 유지 + 콘솔에서 수동으로 파이프라인 정의 롤백 + feature branch에서 `cdk diff` 미리 검증 + Stage별 분리 테스트
C) AWS Support 케이스만으로 복구 대기 — 외부 응답 시간에 종속되어 RTO를 통제할 수 없고 self-mutation 재발도 예방하지 못함
D) 사고 시마다 Pipeline을 삭제 후 재생성 — 복구는 되지만 매번 부트스트랩·권한·아티팩트가 초기화되어 비용·다운타임·휴먼에러가 폭증
