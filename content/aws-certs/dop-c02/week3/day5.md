...
A) Compute Type을 `BUILD_GENERAL1_2XLARGE`로 업그레이드해 vCPU와 메모리를 늘려 npm install 병렬 처리량을 높임
B) S3 Cache로 `node_modules/**/*` 캐시; 또는 BuildKit + ECR Registry Cache로 Docker 레이어에 npm install 포함
C) Local Source Cache와 Local Custom Cache를 함께 활성화해 호스트 디스크에 의존성을 보존
D) npm 대신 yarn berry(PnP)로 전환하고 `--immutable` 플래그로 lockfile 기반 설치 가속
...
A) Aurora를 Public 서브넷으로 이동하고 Security Group을 빌드 호스트 공인 IP로 제한
B) CodeBuild VPC 모드 + Aurora와 같은 VPC + Security Group으로 5432 포트 허용 + Service Role에 ENI 생성 권한
C) 마이그레이션 로직을 Lambda 함수로 옮겨 같은 VPC에 배치하고 CodeBuild가 Invoke로 우회 호출
D) EC2 Bastion을 같은 서브넷에 두고 SSM Run Command로 마이그레이션 SQL을 수동 실행
...
B) Parameter Store Standard SecureString에 저장하고 회전 시 EventBridge로 알림 받아 수동 업데이트
C) buildspec `env.variables`에 평문으로 하드코딩하고 KMS로 빌드 로그를 암호화
D) IAM Database Authentication으로 비밀번호 없이 RDS 토큰 기반 연결 (Aurora + IAM 인증 활성화 필요)
...
B) EventBridge Scheduler로 빌드 시간을 Rate Limit이 6시간마다 리셋되는 시간대로 분산 스케줄링
C) Docker Hub Team 유료 플랜을 구매해 익명 대비 높은 pull 한도를 확보
D) `concurrentBuildLimit`을 1로 두어 동시 pull을 줄이고 Rate Limit 소모 속도를 낮춤
...
A) 4개 별도 CodeBuild 프로젝트를 만들고 CodePipeline에서 순차 Action으로 실행
B) Build Batch `build-list`에 4개 노드 정의 (Java-x86, Java-ARM, Python-x86, Python-ARM) + 병렬 실행
C) 하나의 buildspec의 build 페이즈에서 docker buildx로 4개 조합을 순차적으로 빌드
D) Jenkins 매트릭스 빌드로 위임하고 CodeBuild는 소스 동기화만 담당
...
A) Compute Type을 더 큰 인스턴스로 키워 빌드 자체 실행 시간을 단축해 일일 총 빌드 시간을 줄임
B) Reserved Capacity Fleet으로 워밍업된 컨테이너를 유지해 Provisioning 시간을 거의 0으로 단축
C) 빌드 트리거를 묶어 하루 300번에서 50번으로 빌드 주기를 줄여 Provisioning 횟수 자체를 감소
D) VPC 모드를 비활성화해 ENI 생성에 드는 Provisioning 지연을 제거
...
A) NAT Gateway를 추가해 빌드 컨테이너의 아웃바운드 IP 할당 경로를 늘린다
B) 더 큰 CIDR의 서브넷으로 교체하거나 서브넷을 추가, `concurrentBuildLimit`으로 동시 빌드 수를 제한
C) Internet Gateway를 추가하고 서브넷을 Public으로 전환해 ENI가 공인 IP를 받게 한다
D) Service Role에 `ec2:CreateNetworkInterfacePermission` 권한을 추가해 ENI 생성 한도를 해제한다
...
A) EC2 Bastion을 같은 VPC에 두고 SSH로 빌드 컨테이너의 22번 포트에 접속
B) `--debug-session-enabled`로 빌드 시작 후 SSM Session Manager로 컨테이너에 접속 (최대 7시간)
C) 빌드가 끝날 때까지 기다렸다가 동일 이미지로 새 EC2를 띄워 환경 변수까지 수동 재현
D) CloudWatch Logs와 reports 블록 출력에서 에러 메시지와 스택 트레이스만 확인
...
B) S3에 XML 파일을 Secondary artifact로 업로드하고 Lambda로 파싱해 결과를 SNS로 발행
C) CloudWatch Logs에 XML 텍스트를 출력하고 Metric Filter로 실패 패턴을 감지해 알람 → SNS
D) Secondary artifact로 XML만 저장하고 CodeBuild Test Report 그룹에 수동 연결
...
B) Account B에 IAM User를 만들어 장기 Access Key를 발급하고 Account A의 Secrets Manager에 저장해 빌드가 사용
C) Account A에서 sts:AssumeRole로 Account B의 IAM Role을 assume한 뒤 임시 자격 증명으로 GetSecretValue 호출
D) Account B의 시크릿 값을 KMS 암호화한 채 공유 S3 버킷에 복사하고 빌드가 다운로드

**정답: A**
해설: Secrets Manager는 Resource-based Policy로 크로스 계정 접근을 제어한다. Account B의 시크릿에 Account A CodeBuild Service Role이 `secretsmanager:GetSecretValue`를 허용받아야 하고, 그 시크릿을 암호화하는 KMS 키도 Account A Role에 Decrypt를 허용하는 Key Policy 또는 Grant가 필요하다. B는 장기 자격 증명 사용으로 보안상 좋지 않고, C는 IAM Role assume도 가능하지만 이 시나리오의 Secrets Manager 직접 접근 요건을 충족하려면 추가 설정이 필요하다. D는 시크릿을 복제·동기화해야 하므로 회전 시 값이 어긋나고 단일 진실 공급원 원칙을 깬다.
...
B) IAM Policy에 SBOM 생성 권한을 부여하고 빌드 Service Role이 인벤토리를 자동 산출하게 위임
C) CloudFormation 템플릿에 의존성 목록을 선언해 Stack 배포 시 SBOM을 산출물로 관리
D) CodeArtifact 리포지토리에서 의존성 해석 시 자동 생성되는 SBOM을 빌드가 내려받아 첨부
