...
A) Self-managed StackSets에서 매번 신규 계정 ID를 수동 추가
B) Lambda를 EventBridge로 호출해 각 계정에 boto3로 베이스라인 적용
C) Tooling 계정을 Delegated Administrator로 등록 + Service-managed StackSets + AutoDeployment Enabled로 OU 타겟 지정
D) 모든 계정에 운영자가 SSH로 직접 설정 적용
...
A) `aws cloudformation deploy` 한 줄로 모든 환경에 동일하게 배포
...
B-alt) `--require-approval=any-change` 옵션만
C) IAM 권한만 강화
D) Drift Detection 자동화만
...
C) 모든 Stack을 Nested로 통합
D) Terraform으로 마이그레이션
...
A) StackSets Drift Detection만으로 충분
