# Day 35 - Week 7 복습: 메시징의 네 가지 모델과 분산 시스템의 의사결정
...
A) SQS 큐 4개에 각각 직접 메시지 전송
B) SNS 토픽 → SQS 큐 4개 fanout (SNS-SQS fanout 패턴)
C) Kinesis Data Stream + 각 시스템이 KCL로 consume
D) EventBridge bus + 4개 rule
...
A) SQS Standard + 멱등성 키
B) SQS FIFO + MessageGroupId=customer_id
C) SNS FIFO + Lambda
D) Kinesis Data Streams + partition key=customer_id
...
A) 각 SaaS에 webhook 받는 Lambda를 만들고 SNS로 fanout
B) EventBridge SaaS Partner Source + bus + rules
C) Kinesis Data Streams로 SaaS 이벤트 수집 후 분기
D) Step Functions에 각 SaaS 호출 직접 통합
...
A) SQS + Lambda + S3 export
B) SNS → SQS / Lambda / S3 (fanout)
...
C) Standard 큐의 한계, 멱등성 키만 추가하면 됨
D) DLQ + maxReceiveCount 추가
...
A) CodePipeline → SNS → Lambda(필터링) → Slack webhook
B) CodePipeline → EventBridge rule (실패 이벤트만) → SNS → Chatbot → Slack
C) CodePipeline → SQS → Lambda → Slack
D) CodePipeline → Kinesis → Slack
...
A) Shard 수가 부족함 → shard 추가
B) Classic consumer는 shard당 2MB/s out을 모든 소비자가 공유 → Enhanced Fan-Out 사용
C) Partition key가 hot함 → key 재설계
D) Visibility Timeout이 짧음
...
A) 모든 이벤트를 SQS DLQ로 백업
B) EventBridge Archive + Replay 기능
C) Kinesis로 모든 이벤트 복제
D) S3에 모든 이벤트 직접 적재
...
A) Lambda가 직접 각 푸시 서비스 API 호출
B) SNS Mobile Push (Platform Application + Endpoint)
C) EventBridge → 각 푸시 서비스
D) Kinesis → Lambda → 푸시
...
A) CPU Utilization 기반 Target Tracking
...
A) 600KB를 3개 메시지로 잘라 보내고 소비자가 조립
