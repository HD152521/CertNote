# Day 11 - EC2 인스턴스 유형과 구매 옵션: 하드웨어 설계와 경제학의 교차점
...
**문제 6.** 워크로드 설명에 맞는 인스턴스 패밀리를 연결하시오.

A) 실시간 Apache Kafka 브로커 (높은 디스크 I/O, 낮은 레이턴시 필요)  
B) 딥러닝 모델 사전학습 (GPU 집중)  
C) SAP BW 인메모리 분석 (512GB RAM 필요)  
D) 마이크로서비스 앱 서버 (트래픽 변동 큼)  

1) T 패밀리  2) R 패밀리  3) P 패밀리  4) I 패밀리
