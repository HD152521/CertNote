...
A) 바로 `update-stack`을 실행하고, RDS가 교체되면 자동 생성되는 스냅샷에서 복원해 데이터 손실을 막는다
B) Change Set을 생성하고 `Replacement` 필드를 확인한다. `False`이면 인플레이스 업데이트, `True`이면 RDS 교체 + 데이터 손실 위험
C) 먼저 RDS 수동 스냅샷을 만든 뒤 `update-stack`을 실행하면 교체가 발생해도 스냅샷이 있어 안전하다
D) Stack을 삭제하고 새 인스턴스 클래스로 재생성하면 깨끗한 상태에서 안전하게 타입을 바꿀 수 있다
...
A) `update-stack`으로 새 Template를 적용해 ROLLBACK_COMPLETE 상태를 정상 상태로 덮어쓴다
B) `create-change-set`으로 변경 사항을 미리 확인한 뒤 `execute-change-set`으로 적용한다
C) `delete-stack`으로 삭제한 후 `create-stack`으로 재생성한다
D) `continue-update-rollback`으로 미완료 롤백을 마저 진행해 Stack을 복구한 뒤 업데이트한다
...
A) 50개 계정 각각에 수동으로 CloudFormation Stack을 배포하고, 신규 계정은 온보딩 체크리스트에 Stack 배포 단계를 추가해 누락을 방지한다
B) StackSets with Service-Managed Permissions + `AutoDeployment=true`를 설정한다
C) EventBridge로 Organizations `CreateAccountResult` 이벤트를 감지해 Lambda가 신규 계정에 CloudFormation Stack을 배포하도록 자동화한다
D) AWS Control Tower로 전환해 Account Factory가 신규 계정 프로비저닝 시 보안 Baseline을 자동 적용하게 한다

**정답: B**
해설: StackSets Service-Managed + AutoDeployment=true가 정확한 답이다. Organizations와 네이티브로 통합되어 새 계정이 OU에 추가될 때 자동으로 Stack Instance가 생성된다. C도 동작하지만 커스텀 EventBridge·Lambda 유지가 필요해 오버헤드가 크다. D의 Control Tower도 신규 계정 Baseline 자동화를 제공하지만, 이미 50개 계정이 운영 중인 환경을 Control Tower로 전환하는 것은 훨씬 큰 작업이라 "일괄 배포 + 신규 자동 포함"이라는 요구에는 StackSets Service-Managed가 가장 직접적이다.

---

**문제 4.** 운영팀이 Production Stack을 실수로 삭제하는 것을 방지하려 한다. 어떤 방법이 가장 직접적인가?

A) 운영자 IAM 정책에서 `cloudformation:DeleteStack`을 Deny하고, prod Stack ARN을 조건으로 걸어 삭제를 차단한다
B) Stack에 Termination Protection을 활성화한다
C) Stack Policy에 `"Effect":"Deny","Action":"Update:Delete","Resource":"*"`를 추가해 모든 리소스 삭제를 차단한다
D) `cloudformation:DeleteStack` 호출에 `aws:MultiFactorAuthPresent: true` 조건을 걸어 MFA 없이는 삭제를 막는다
...
A) 개발자에게 시간 제한이 걸린 임시 RDS 생성 권한을 STS로 부여하고, CloudTrail로 사용 이력을 감사한다
B) Service Catalog Portfolio에 RDS 제품을 등록하고, Launch Constraint에 RDS 생성 권한이 있는 IAM Role을 지정한다
C) 표준 RDS CloudFormation Template를 S3에 두고 개발자가 직접 `create-stack`으로 배포하도록 `cloudformation:CreateStack`만 허용한다
D) 개발자에게 RDS Multi-AZ·암호화 등 표준 설정을 문서로 교육하고 콘솔에서 직접 생성하게 한다
...
A) Parameter Store에 Flag 값을 두고 Lambda가 매 호출마다 읽게 한 뒤, 에러율 알람이 울리면 운영자가 값을 되돌린다
B) AppConfig + Feature Flag 타입 프로파일 + Canary 배포 전략 + CloudWatch Alarm Monitor 설정
C) DynamoDB 테이블에 Flag 값을 저장하고 DynamoDB Streams + Lambda로 점진 토글·에러 시 롤백을 직접 구현한다
D) Lambda 환경 변수에 Flag를 두고 CodeDeploy Canary로 별칭 트래픽을 점진 이동시키며 알람 시 롤백한다

**정답: B**
해설: AppConfig가 정확한 답이다. Feature Flag 타입 프로파일로 Flag를 정의하고, Canary(또는 Linear) 배포 전략으로 점진적으로 적용한다. Environment에 CloudWatch Alarm Monitor를 연결하면 배포 중 알람 발생 시 자동 롤백된다. Parameter Store(A)도 값 저장은 가능하지만 점진적 배포와 자동 롤백 기능이 없어 수동 개입이 필요하다. DynamoDB(C)는 Streams로 직접 구현해야 해 오버헤드가 크다. D는 CodeDeploy Canary로 코드 배포(별칭 트래픽)는 점진화하지만, 환경 변수 변경 자체가 함수 재배포를 요구해 "코드 재배포 없이 Flag 토글"이라는 요구에 어긋난다. AppConfig는 코드 재배포 없이 Flag 값만 점진 배포·자동 롤백한다는 점이 결정적이다.

---

**문제 7.** 누군가 콘솔에서 CloudFormation으로 관리되는 보안 그룹에 `0.0.0.0/0:22` 규칙을 직접 추가했다. 이 변경을 자동으로 감지해 알림을 받으려면?

A) `AuthorizeSecurityGroupIngress` API를 추적하는 CloudWatch Metric Filter + Metric Alarm으로 SG 규칙 추가를 감지해 알림한다
B) Drift Detection을 EventBridge Scheduled Rule + Lambda로 주기적으로 실행하고, MODIFIED 리소스 발견 시 SNS로 알림을 발송한다
C) GuardDuty가 0.0.0.0/0:22 같은 개방 규칙을 의심스러운 구성 변경으로 자동 Finding을 생성한다
D) AWS Config Rule `restricted-ssh`로 SG를 평가해 비준수 시 알림하고, Inspector로 EC2 노출을 함께 스캔한다

**정답: B**
해설: Drift Detection이 CFn 관리 리소스의 Template 외부 변경을 감지한다. CFn Drift Detection은 자동으로 실행되지 않으므로, EventBridge Scheduled Rule(cron)로 Lambda를 주기적으로 트리거해 `detect-stack-drift`를 실행하고, 결과가 MODIFIED이면 SNS로 알림을 발송하는 자동화가 필요하다. CloudTrail(언급되지 않은 선택지)은 "누가 언제 추가했는가"를 알 수 있지만 현재 상태의 drift를 표시하지 않는다. GuardDuty(C)는 위협 탐지이고 SG 직접 변경은 감지 대상이 아니다. A·D(Metric Filter, Config `restricted-ssh`)도 "0.0.0.0/0:22 규칙"이라는 특정 위반은 잡지만, 이 문제의 핵심은 "CFn Template과 실제 상태의 괴리(drift)"를 감지하는 것이므로 Template 기준 비교를 수행하는 Drift Detection이 정답이다. Config Rule은 규칙 위반 여부만 알 뿐 Template과의 drift는 표시하지 않는다.
...
A) Stack A만 업데이트하면 Export 값이 갱신되고 Stack B, C가 다음 drift 감지 시 자동으로 새 VPC ID를 참조한다
B) Export는 참조 무결성을 자동 관리하므로 Stack A를 바로 업데이트하면 CloudFormation이 B, C를 순서대로 함께 갱신한다
C) Stack B와 Stack C에서 `!ImportValue`를 제거하고, Stack A를 업데이트한 후, Stack B와 Stack C를 새 Export 값으로 업데이트한다
D) Stack A를 삭제 후 재생성해 Export를 새 VPC ID로 다시 만들면 B, C가 끊김 없이 새 Export에 다시 연결된다
...
A) EventBridge 스케줄로 Lambda가 5xx CloudWatch Alarm 상태를 폴링하다가 ALARM이면 `cancel-update-stack`을 호출해 직전 상태로 되돌린다
...
C) CodeDeploy Blue/Green과 CloudFormation을 통합해 5xx 알람 발생 시 트래픽을 원래 환경으로 자동 전환한다
D) CloudFormation 콘솔에서 "Enable Auto Rollback" 옵션을 켜고 임계 알람을 연결해 업데이트 후 10분간 감시하게 한다
...
A) Mappings 섹션에 리전별 최신 AMI ID를 넣고, EventBridge 스케줄 + Lambda로 매월 Mapping을 자동 갱신해 하드코딩 부담을 줄인다
B) Lambda 기반 Custom Resource를 두어 Stack 배포 시마다 `ec2:DescribeImages`로 최신 Amazon Linux 2 AMI를 조회해 반환하게 한다
...
D) CloudFormation 콘솔의 "Use Latest AMI" 옵션과 `Fn::LatestAMI` 내장 함수를 함께 써서 배포 시 최신 이미지를 자동 선택한다

**정답: C**
해설: AWS는 `/aws/service/ami-amazon-linux-latest/...` SSM 경로에 최신 AMI ID를 자동으로 업데이트한다. Parameter 타입을 `AWS::SSM::Parameter::Value<AWS::EC2::Image::Id>`로 설정하면 CloudFormation이 배포 시점에 자동으로 최신 AMI ID를 가져온다. 수동 업데이트 없이 항상 최신 AMI를 사용할 수 있는 운영 모범 사례다. B의 Lambda Custom Resource도 최신 AMI를 가져올 수 있지만 Lambda 코드·권한·실패 처리를 직접 유지해야 해 SSM Parameter 타입보다 과하다. D의 "Use Latest AMI" 옵션이나 `Fn::LatestAMI` 함수는 존재하지 않는다.

---

**문제 11.** AppConfig Deployment Strategy에서 `FinalBakeTimeInMinutes: 15`를 설정했다. `AllAtOnce` 전략으로 배포했을 때 정확한 동작은?

A) `AllAtOnce`라도 FinalBakeTime이 있으면 15분에 걸쳐 트래픽이 선형으로 새 설정에 점진 적용된다
B) 설정이 즉시 100% 적용되고, 이후 15분 동안 CloudWatch Alarm을 감시한다. 알람 발생 시 자동 롤백된다
C) FinalBakeTime이 시작 지연으로 작용해 배포가 15분 대기 후에 비로소 100% 적용된다
D) GrowthFactor 기본값에 따라 배포가 10%씩 15분 간격으로 단계 적용된다
...
A) Permission Boundary가 Launch Role의 정책 평가 범위를 좁혀 CloudFormation 프로비저닝 실행 속도를 높이기 위해
B) Launch Role이 Service Catalog의 Provision/Terminate API를 호출할 수 있도록 필요한 신뢰 관계를 부여하기 위해
C) 사용자가 Service Catalog를 통해 Launch Role보다 강력한 권한(예: Admin)으로 리소스를 만드는 권한 에스컬레이션을 방지하기 위해
D) Launch Role을 멀티 계정·멀티 리전 StackSet 배포에 사용할 수 있도록 경계를 확장하기 위해
