...
C) Security Group이 차단
D) 라우팅 테이블에 0.0.0.0/0 경로 없음
...
C) NAT Instance 한 대로 충분
D) Internet Gateway만으로 가능
...
A) Prod와 Dev VPC에 VPC Peering 추가
B) TGW Route Table을 분리 — Prod RT(Shared만 propagate), Dev RT(Shared만 propagate), 상호 격리
C) Prod VPC의 SG에서 Dev VPC CIDR 차단
D) TGW를 prod와 dev로 두 개 만듦
