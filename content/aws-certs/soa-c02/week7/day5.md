...
C) CloudFormation StackSet으로 전체 ASG 재생성
D) Lambda가 모든 인스턴스에 yum update를 SSH로 실행
...
A) RDS에 DeletionPolicy: Retain 추가 (Beanstalk 콘솔에선 직접 설정 불가)
...
C) 별도 ASG 2개 (On-demand 전용, Spot 전용)
D) EC2 Fleet만으로 단일 호출
...
A) 별도 Lambda를 만들어 CloudWatch Events에서 트리거
