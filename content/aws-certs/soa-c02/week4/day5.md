...
A) CloudWatch Alarm + Metric Filter
B) EventBridge Rule (CloudTrail 이벤트 패턴 `userIdentity.type = Root`) → SNS + Lambda(Slack)
C) Config Rule
D) Audit Manager

**정답: B**
해설: CloudTrail은 모든 API 호출(콘솔 로그인 포함)을 EventBridge default bus로 자동 전달한다. EventBridge Rule에서 `userIdentity.type = Root` 패턴을 매칭하면 Root 로그인만 필터링된다. SNS와 Lambda(Slack webhook 호출)를 Target으로 지정하면 실시간 다채널 알림이 가능하다. Config Rule은 리소스 상태를 평가하는 도구이므로 실시간 이벤트 대응에 부적합하다.
...
C) GuardDuty S3 Threat Detection
D) Amazon Macie
...
A) Config 결과를 CSV로 내보내 수동으로 정리
B) Audit Manager의 사전 제공 HIPAA Framework로 Assessment 실행 → 자동 증거 수집 → PDF/CSV 보고서
C) CloudTrail 로그를 엑셀로 변환
D) Trusted Advisor 점검 결과 캡처
...
C) CloudWatch Cross-Account Dashboard
D) CloudTrail Lake Organization EDS
