...
A) 파트너별로 별도의 REST API와 스테이지를 생성하고 스테이지 레벨 스로틀링을 설정한다  
B) 파트너별 사용량 플랜(Usage Plan)을 생성하고 각 파트너에게 API 키를 발급해 해당 플랜에 연결한다  
C) Lambda Authorizer에서 파트너 ID를 확인하고 호출 횟수를 DynamoDB에 기록해 제한한다  
D) API Gateway 리소스 정책으로 파트너 IP별 요청 수를 제한한다
...
A) API Gateway Authorizer가 느리게 응답하고 있다  
B) Lambda 함수가 응답하기 전에 API Gateway 통합 타임아웃(29초)에 도달하고 있다  
C) 클라이언트가 너무 많은 요청을 보내 스로틀링이 발생하고 있다  
D) CloudWatch Logs 설정이 잘못되어 메트릭이 왜곡되고 있다
...
A) Lambda 함수를 추가해 DynamoDB 응답을 파싱한다  
B) DynamoDB 테이블 타입 설정을 변경해 문자열로만 저장한다  
C) Integration Response의 VTL 매핑 템플릿을 작성해 DynamoDB 응답을 클린 JSON으로 변환한다  
D) HTTP API로 마이그레이션하면 DynamoDB가 자동으로 타입을 제거한다
...
B) 캐싱은 사용자 기반 분리를 지원하지 않으므로 Lambda에서 캐싱 로직을 구현한다  
C) Cognito Authorizer 대신 Lambda Authorizer를 사용하면 자동으로 사용자별 캐시가 분리된다  
D) REST API 대신 HTTP API를 사용한다
...
A) WebSocket API + DynamoDB 연결 레지스트리 + Lambda 브로드캐스트  
B) REST API + 클라이언트 폴링(5초 간격)  
C) Lambda Response Streaming + Function URL (SSE 패턴)  
D) SNS + SQS + Lambda + REST API 롱 폴링
...
A) WAF Sampled Requests를 활성화해 차단된 요청의 샘플을 CloudWatch에 기록한다  
B) API Gateway Execution Logs를 INFO 레벨로 설정한다  
C) X-Ray를 활성화한다  
...
A) REST API (Cognito User Pool Authorizer + 수동 CORS)  
B) HTTP API (JWT Authorizer + 자동 CORS 설정 + 커스텀 도메인)  
C) Function URL (자동 CORS + IAM 인증)  
D) WebSocket API
...
A) Cognito에서 해당 사용자 계정을 비활성화한다  
B) Lambda Authorizer 함수 코드에서 해당 토큰을 블랙리스트에 추가하고, Authorizer TTL을 0으로 임시 변경한다  
C) TTL이 만료될 때까지 300초를 기다린다  
D) API Gateway 스테이지를 재배포한다
...
A) Scan + FilterExpression으로 전체 테이블에서 해당 채팅방 연결만 필터링한다  
B) 채팅방 ID를 GSI Partition Key로 설정하고 Query를 사용해 해당 채팅방의 연결만 조회한다  
C) 채팅방별로 별도 DynamoDB 테이블을 생성한다  
D) ElastiCache Redis에 채팅방-연결 매핑을 저장하고 조회한다
