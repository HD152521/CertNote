# Day 35 - Week 7 종합: 데이터 계층 의사결정의 모든 것
...
A) Lambda 환경변수에 비밀번호 + reserved concurrency로 동시성 제한
B) RDS IAM Authentication 단독 사용
...
A) RDS Multi-AZ DB Instance Deployment + Read Replica 2개
B) RDS Multi-AZ DB Cluster Deployment (2022 신규)
C) Aurora MySQL Multi-AZ
D) RDS Single-AZ + Backup
...
A) PITR로 새 인스턴스 생성 후 데이터 export/import
...
A) DynamoDB GSI에 점수 인덱스
B) RDS MySQL + ORDER BY
...
C) Lazy Loading 제거하고 모든 조회를 DB로
D) Memcached로 전환
...
A) Aurora Cross-Region Read Replica 2개 (미국, 유럽)
B) Aurora Global Database (Primary: 도쿄, Secondary: 버지니아, 아일랜드)
C) DynamoDB Global Tables
D) Aurora Multi-AZ만 사용
...
A) RDS Single-AZ db.t3.micro + Reserved Instance 1년
B) RDS Multi-AZ + Auto Scaling
...
A) `preferred-maintenance-window`를 주말 새벽으로 명시 설정
B) `auto-minor-version-upgrade`를 false로 변경하고 수동 패치 일정 관리
C) RDS Proxy를 도입해 connection 안정성 확보
D) Multi-AZ Cluster Deployment로 전환 (페일오버 35초 이내)
...
A) DynamoDB read latency를 ms → μs로 단축
B) DynamoDB read 비용을 절감 (캐시 hit으로 RCU 소비 감소)
C) ElastiCache Redis의 범용 캐시로 사용
D) DynamoDB Eventually Consistent Read의 응답 시간 개선
