...
- **소량·단발 → S3 CLI/SDK**. 대용량은 멀티파트 업로드를 쓰고, Lifecycle `AbortIncompleteMultipartUpload`로 미완료 조각을 정리한다.
- **대량·반복·검증 필요 → DataSync**. 스케줄 증분 동기화 + 체크섬 무결성 검증 + 메타데이터 보존.
- **외부 파트너가 SFTP/FTPS로 밀어넣음 → Transfer Family**. 클라이언트·인증은 그대로 두고 백엔드만 S3로 바꾼다.
- **페타바이트급·느린 회선·일회성 → Snow 패밀리**. 물리 배송이라 회선을 점유하지 않는다.
- 이 네 가지 선택지의 경계를 키워드로 구분하는 능력이 DEA-C01 수집 도메인의 기본기다.

## S3는 데이터 레이크의 착륙장이다

- 배치 수집은 "정해진 단위(파일·테이블·시간)로 모아서 한꺼번에" 가져오는 방식이다. 실시간성이 필요 없는 야간 적재, 온프레미스 마이그레이션, 대용량 일괄 전송이 여기 속한다.
- AWS는 데이터의 양과 위치, 네트워크 상황에 따라 서로 다른 도구를 제공한다. 오늘 다룰 4가지는 S3 업로드 패턴, DataSync, Transfer Family, Snow 패밀리다.
- 대부분의 AWS 분석 파이프라인에서 데이터가 처음 도착하는 곳은 S3다. 사실상 무제한 용량, 11 9's 내구성, 그리고 Athena·Glue·Redshift Spectrum·EMR이 모두 직접 읽을 수 있는 공통 저장소이기 때문이다.
- 그래서 "배치 수집 = 어떻게든 S3에 파일을 안전하게 올리기"로 단순화해도 크게 틀리지 않는다.
- 작은 파일은 단순 PutObject로 충분하다.
- 큰 파일(수백 MB 이상)은 **멀티파트 업로드(Multipart Upload)** 를 써야 한다. 파일을 여러 조각으로 나눠 병렬 전송하고, 한 조각이 실패해도 그 조각만 재전송하면 되므로 대용량 전송이 빠르고 견고해진다.
...
S3에 도착하는 순간 이벤트를 발생시켜 다운스트림을 트리거할 수도 있다.

- **S3 Event Notification**: 객체 생성 이벤트를 Lambda·SQS·SNS·EventBridge로 보낸다.
- prefix/suffix 필터를 걸면 "파일이 들어오면 바로 Glue 잡 실행" 같은 이벤트 드리븐 배치가 된다.
...
- **AWS DataSync**는 온프레미스 NAS·파일 서버에 쌓인 수 TB의 데이터를 S3·EFS·FSx로 옮기는 작업을 자동화·가속하는 관리형 전송 서비스다.
- 온프레미스에 에이전트(VM)를 두고, AWS와의 사이에서 데이터를 멀티스레드로 전송한다.
- 전송 중·후에 **체크섬으로 무결성을 검증**한다.
- 핵심 가치는 단발성 복사가 아니라 **반복 가능한 스케줄 동기화**다. 매일 밤 변경된 파일만 증분 전송하도록 예약해, 온프레미스에서 데이터 레이크로의 지속적 피드 파이프라인을 만든다.
...
- 수많은 기업·파트너 시스템은 여전히 **SFTP/FTPS/FTP**로 파일을 주고받는다.
- **AWS Transfer Family**는 이 레거시 프로토콜을 유지하면서 저장소만 S3로 바꿔주는 관리형 게이트웨이다. 외부 파트너는 평소처럼 SFTP로 올리지만, 그 파일은 곧장 S3 버킷(또는 EFS)에 저장된다.
- 핵심은 "파트너의 클라이언트와 인증 방식은 그대로 두고, 백엔드만 S3로 교체"한다는 점이다.
- 기존 EDI·B2B 파일 교환을 코드 변경 없이 데이터 레이크에 연결할 수 있다.
...
- DataSync: 내가 AWS로 끌어오는(pull/push) 대량 동기화
- Transfer Family: 외부가 정해진 프로토콜로 밀어 넣는(receive) 게이트웨이
...
- 장비를 받아 데이터를 채운 뒤 반송하면 AWS가 S3로 적재한다.
- 전송 중에는 256비트 암호화로 보호된다.
...
- **배치 수집** : 데이터를 파일·테이블·시간 단위로 모아 한꺼번에 가져오는 방식. 실시간이 아니어도 되는 야간 적재 같은 작업에 쓴다.
- **데이터 레이크** : 정제 전 원본 데이터까지 그대로 모아두는 대형 저장소. AWS에서는 보통 S3가 그 역할을 한다.
- **멀티파트 업로드** : 큰 파일을 여러 조각으로 쪼개 동시에 올리는 방식. 실패해도 그 조각만 다시 보내면 된다.
- **AbortIncompleteMultipartUpload** : 완료되지 않고 남은 업로드 조각을 며칠 뒤 자동으로 지우는 S3 Lifecycle 설정. 안 걸어두면 안 보이는 요금이 쌓인다.
- **11 9's 내구성** : 99.999999999%. 저장한 객체가 사실상 사라지지 않는다는 뜻의 S3 내구성 수치.
- **S3 Event Notification** : 버킷에 객체가 생기면 Lambda·SQS·SNS·EventBridge로 알림을 보내는 기능. 파일 도착을 신호로 다음 작업을 자동 실행할 때 쓴다.
- **증분 전송** : 전체를 다시 보내지 않고 지난번 이후 바뀐 것만 골라 보내는 방식.
- **체크섬** : 데이터를 계산해 얻은 짧은 지문값. 보내기 전후 값을 비교해 파일이 깨지지 않았는지 확인한다.
- **SFTP/FTPS** : 파일을 안전하게 주고받는 오래된 표준 프로토콜. 기업 간 파일 교환에 여전히 많이 쓰인다.
- **온프레미스** : 클라우드가 아니라 회사가 직접 소유·운영하는 자체 서버실·장비 환경.
