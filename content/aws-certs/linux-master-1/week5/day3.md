...
- LVM은 디스크와 볼륨 사이에 **추상화 계층**을 끼워, 물리적 제약에서 논리적 사용을 분리한다.
- 3계층: **PV(디스크) → VG(풀) → LV(사용 단위)**. 생성은 `pvcreate` → `vgcreate` → `lvcreate`.
- 확장은 **2단계** — `lvextend`(그릇) + `resize2fs`/`xfs_growfs`(내용물). VG가 모자라면 `vgextend`가 먼저다.
- **xfs는 축소 불가**, `xfs_growfs`는 마운트포인트를 받는다. ext4 `resize2fs`는 온라인 확장 가능·축소는 언마운트 후.
- 스왑은 파일시스템이 아니다 — `mkswap`으로 초기화하고 **`swapon`**으로 켜며, fstab에는 `none swap sw 0 0`.

## LVM의 3계층: PV → VG → LV

전통적 파티션은 한 번 크기를 정하면 늘리기가 까다롭다. 옆 파티션에 여유가 있어도 물리적으로 인접하지 않으면 빌려올 수 없다. LVM은 물리 디스크들을 하나의 저장 풀로 묶어 이 경직성을 깬다.
...
- 흐름: 디스크/파티션을 `pvcreate`로 PV로 → 여러 PV를 `vgcreate`로 묶어 VG로 → VG에서 `lvcreate`로 LV를 잘라냄 → LV를 `mkfs`로 포맷하고 마운트해서 사용.
...
- **스왑**은 물리 메모리(RAM)가 부족할 때 당장 안 쓰는 메모리 페이지를 디스크로 잠시 내보내(swap out) RAM을 확보하는 영역이다.
- 스왑은 **파일시스템이 아니다** — 마운트하지 않고, 전용 포맷(`mkswap`)으로 초기화한 뒤 커널에 등록(`swapon`)해서 쓴다.
...
다음 Day 4에서는 RAID 레벨별 패리티와 mdadm, 디스크 쿼터, 그리고 fsck·df·du·lsblk·blkid 점검 도구를 다룬다.

## 📖 용어

- **LVM** : 디스크와 볼륨 사이에 추상화 계층을 넣어 무중단 확장을 가능하게 하는 논리 볼륨 관리자.
- **PV(물리 볼륨)** : LVM에 편입된 디스크나 파티션. 풀에 붓는 물병에 해당한다.
- **VG(볼륨 그룹)** : PV들을 묶은 저장 풀. 여기서 공간을 떼어 LV를 만든다.
- **LV(논리 볼륨)** : VG에서 잘라낸 실제 사용 단위. 포맷하고 마운트해서 쓴다.
- **PE(Physical Extent)** : VG가 공간을 관리하는 고정 크기 덩어리(기본 4MB). LV 크기는 PE의 정수배다.
- **`-L` vs `-l`** : 용량으로 지정(`-L 10G`)과 PE 개수·백분율로 지정(`-l 100%FREE`).
- **`/dev/mapper/VG-LV`** : `/dev/VG/LV`와 같은 대상을 가리키는 또 다른 경로 표기.
- **`lvextend` + `resize2fs`** : 그릇을 키우는 단계와 그 안의 파일시스템을 키우는 단계. 둘 다 해야 `df`에 반영된다.
- **`xfs_growfs`** : xfs 전용 확장 명령. **마운트포인트**를 인자로 받고, 축소는 불가능하다.
- **`vgextend`** : VG에 새 PV를 편입해 풀을 키우는 명령. LV 확장 전에 여유가 없으면 이게 먼저다.
- **LVM 스냅샷** : `lvcreate -s`로 특정 시점을 동결하는 기능. CoW라 변경분만 공간을 차지한다.
- **스왑(swap)** : RAM이 부족할 때 쓰지 않는 페이지를 대피시키는 디스크 영역. 파일시스템이 아니다.
- **`mkswap` / `swapon` / `swapoff`** : 스왑 영역 초기화 / 활성화 / 비활성화.
- **`vm.swappiness`** : 커널이 얼마나 적극적으로 스왑을 쓸지 정하는 값(0~100, 기본 60).
- **하이버네이션** : RAM 내용을 통째로 디스크에 저장하는 기능. 그래서 스왑이 RAM 이상이어야 한다.
