...
Week 5는 "디스크 한 장이 어떻게 사용 가능한 저장 공간이 되는가"라는 하나의 큰 흐름을 다섯 날에 걸쳐 따라왔다. 오늘은 그 전체 지도를 한눈에 다시 그리고, 날들을 가로지르는 연결고리를 짚은 뒤, 실기 단답에 대비한 연습문제로 마무리한다.

핵심 흐름을 한 문장으로 압축하면: **물리 디스크 → 파티션 → (선택: RAID/LVM) → 파일시스템(mkfs) → 마운트(mount/fstab) → 운영(쿼터·점검)**. 이 파이프라인의 각 단계에 어떤 개념과 명령이 붙는지를 떠올릴 수 있으면 Week 5는 정복한 것이다.
...
Week 5는 저장 관리의 전 계층을 관통했다. **inode·블록·저널링**(데이터 표현) → **파티션·mkfs·fstab**(공간 구획과 연결) → **LVM·스왑**(유연성과 가상메모리) → **RAID·쿼터·점검**(안전성·공정성·진단). 실기에서는 fstab 6필드, LVM 3계층과 확장 2단계, RAID 레벨별 최소 디스크·패리티, 쿼터 soft/hard, df vs du, fsck 언마운트 조건이 반복 출제된다. 명령을 외우되 "어느 계층에서 무엇을 푸는가"의 맥락으로 묶어 기억하면 변형 문제에도 흔들리지 않는다. 아래 12문항으로 점검하자.
