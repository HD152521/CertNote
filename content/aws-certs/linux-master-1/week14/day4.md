...
- 1급은 **1차 필기 합격 후 2차 실기**까지 통과해야 최종 합격이다. 2차는 **단답형·작업형**으로 직접 써야 한다.
- 필기처럼 "고르는" 게 아니라 "쓰는" 시험이라 **정확한 철자와 옵션**이 곧 점수다. 보기 형태로 연습하되 손으로 쓰는 훈련이 필요하다.
- 작업형 출제 축은 6영역 — 계정·권한, 텍스트 처리, 디스크·마운트, 스케줄링, 네트워크·서비스 설정, systemctl.
- **"지금" 대 "영구"**의 구분이 감점 1순위다 — `mount` 대 `/etc/fstab`, `start` 대 `enable`, `--permanent` 없는 firewall-cmd.
- 권한 계산은 rwx=7·rw-=6·r-x=5·r--=4에, 특수 권한(SetUID 4·SetGID 2·Sticky 1)을 맨 앞 자리에 더한다.
- `/etc/exports`의 **괄호 앞 공백**과 셸 변수 대입의 **`=` 양옆 공백**은 대표적인 실기 감점 함정이다.
...
- 예: `ps -ef | grep nginx | grep -v grep | awk '{print $2}'` — nginx의 PID만 추출한다.
...
철자와 옵션을 손으로 반복 연습해야 한다. 아래 모의로 점검하자.

## 📖 용어

- **단답형 / 작업형** : 명령·옵션·경로를 짧게 써내는 유형 / 시나리오를 명령 절차로 풀어내는 유형.
- **`useradd -m -s`** : 홈 디렉터리를 만들고(`-m`) 로그인 셸을 지정해(`-s`) 계정을 만드는 옵션 조합.
- **`userdel -r`** : 계정과 함께 홈 디렉터리·메일함까지 지우는 옵션.
- **`-exec {} \;`** : find가 찾은 파일마다 명령을 실행하는 구문. `{}`가 파일 이름 자리다.
- **영구 마운트** : `/etc/fstab`에 등록해 재부팅 후에도 유지되는 마운트. `mount`만 하면 임시다.
- **fstab 6필드** : 장치 · 마운트점 · 타입 · 옵션 · dump · fsck 순서.
- **`enable` vs `start`** : 부팅 시 자동 시작 등록 / 지금 즉시 기동. 둘 다 필요하면 `enable --now`.
- **`--permanent` + `--reload`** : firewalld 설정을 영구 저장하고 현재 세션에 반영하는 두 단계.
- **journalctl** : systemd가 모은 로그를 보는 명령. `-u`(서비스), `-b`(이번 부팅), `-f`(실시간).
- **특수 권한 자리** : chmod 네 자리 중 맨 앞. SetUID 4, SetGID 2, Sticky 1을 더해 쓴다(`/tmp`는 1777).
- **exports 공백 함정** : 클라이언트와 괄호 사이를 띄우면 모든 호스트에 권한이 열리는 NFS 문법 함정.
- **파이프(`|`)** : 앞 명령의 출력을 뒤 명령의 입력으로 넘기는 연결. 필터를 이어 붙이는 사고가 실기의 핵심이다.
- **단일 사용자 모드** : GRUB에서 `single`·`rd.break`로 진입하는 복구 모드. root 비밀번호 재설정의 표준 절차다.
