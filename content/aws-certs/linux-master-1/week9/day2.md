...
새 노트북을 회사 와이파이에 연결하면, 아무것도 입력하지 않았는데도 IP 주소·게이트웨이·DNS가 자동으로 채워진다. 이 마법을 일으키는 것이 **DHCP(Dynamic Host Configuration Protocol)**다. 한편 분산 시스템에서는 모든 서버의 시계가 같아야 로그가 맞물리고, 인증서가 유효하며, 데이터베이스 복제가 어긋나지 않는다. 이 시간 동기화를 담당하는 것이 **NTP(Network Time Protocol)**와 그 현대적 구현 **chrony**다. 오늘은 IP 자동 배분의 메커니즘(DORA 4단계)과 `dhcpd.conf`의 핵심 지시어, 그리고 시간 동기화의 계층(stratum)과 chrony 설정을 정밀하게 익힌다.
...
DHCP는 IP뿐 아니라 **서브넷 마스크, 기본 게이트웨이, DNS 서버, 도메인 이름** 같은 네트워크 설정 일체를 함께 내려 준다. 클라이언트는 IP 하나만 받는 게 아니라 "네트워크에 참여하는 데 필요한 모든 좌표"를 한 번에 받는다.
...
클라이언트가 IP를 받는 과정은 네 단계로 이루어지며, 각 단계의 머리글자를 따 **DORA**라 부른다. 시험 단골이므로 순서와 방향을 정확히 외운다.
...
처음 DISCOVER는 클라이언트가 아직 IP가 없으므로 **브로드캐스트**(255.255.255.255)로 나간다. 여러 DHCP 서버가 있으면 각각 OFFER를 보낼 수 있는데, 클라이언트는 보통 가장 먼저 도착한 제안을 택하고, REQUEST에 "어느 서버의 제안을 받아들였는지"를 명시해 나머지 서버에게는 거절을 알린다.
...
리눅스 DHCP 서버 패키지는 `dhcp`이고 데몬은 `dhcpd`, 설정 파일은 `/etc/dhcp/dhcpd.conf`다.
...
리눅스가 169.254.x.x 대역(APIPA, 링크 로컬)의 IP를 들고 있다면, 이는 "DHCP 서버를 찾지 못했다"는 신호다. 이 경우 DHCP 서버·릴레이·네트워크 연결을 점검해야 한다.

## NTP와 시간 동기화 — 왜 시계를 맞춰야 하는가

서버들의 시계가 어긋나면 무슨 일이 벌어질까. 로그의 시간순서가 뒤엉켜 장애 추적이 불가능해지고, TLS 인증서가 "아직 유효하지 않음" 또는 "만료됨"으로 오판되며, 시간 기반 인증(Kerberos, TOTP)이 실패하고, 데이터베이스 복제·분산 트랜잭션이 깨진다. 그래서 **NTP**로 모든 노드를 신뢰할 수 있는 기준 시계에 맞춘다.
...
전통적 `ntpd`를 대체해 현재 RHEL/CentOS 계열의 기본 시간 동기화 데몬은 **chrony**다. 노트북처럼 자주 끊기거나 네트워크가 불안정한 환경에서 더 빠르고 정확하게 동기화하도록 설계됐다.
...
`timedatectl` 출력에서 `NTP synchronized: yes`가 보이면 시간이 정상 동기화된 것이다. `chronyc tracking`의 `Stratum` 값과 `Last offset`(현재 오차)을 함께 보면 동기화 품질을 가늠할 수 있다.

## 마무리

오늘은 네트워크에 "공간"과 "시간"을 자동 배포하는 두 서비스를 익혔다. **DHCP는 DORA(Discover-Offer-Request-Ack) 4단계로 IP와 게이트웨이·DNS를 임대 형태로 나눠 주며**, `dhcpd.conf`의 `range`가 동적 풀을, `host`+`hardware ethernet`+`fixed-address`가 MAC 기반 고정 예약을 정의한다. 브로드캐스트로 출발하기에 라우터 너머는 릴레이가 필요하다는 점도 핵심이다. **NTP는 stratum 계층으로 시간 신뢰도를 표현하며**(숫자가 작을수록 정확, 0은 기준 시계, 16은 미동기), 현대 리눅스는 chrony로 이를 구현한다. `chrony.conf`의 `server/pool`이 소스를, `allow`가 클라이언트 범위를 정하고, `chronyc sources`·`chronyc tracking`·`timedatectl`로 상태를 본다. 내일은 이 인프라 위에서 실제 콘텐츠를 서비스하는 웹 서버, Apache로 넘어간다.
