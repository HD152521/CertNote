...
- 특수 권한은 9비트 앞에 붙는 **네 번째 8진수 자리** — SetUID=4, SetGID=2, Sticky=1.
- **SetUID**는 누가 실행하든 소유자 권한으로 돌린다(`passwd`=4755). 최대의 권한 상승 위험 지점.
- **SetGID 디렉터리**는 그 안에 만든 파일의 그룹을 자동 상속시킨다. 그룹 협업의 핵심 도구.
- **Sticky bit**는 공유 디렉터리에서 남의 파일 삭제를 막는다. `/tmp = 1777`.
- **ACL**은 owner-group-other 세 칸 한계를 넘는다. `ls -l` 끝의 `+`가 존재 신호이고, **mask가 상한선**이다.

## 특수 권한 — 네 번째 8진수 자리

9비트만으로는 풀 수 없는 문제들이 있다. 일반 사용자가 root만 쓸 수 있는 `/etc/shadow`를 어떻게 고치는가? `/tmp`는 모두 쓸 수 있는데 왜 남의 파일은 못 지우는가? 두 사람에게만 서로 다른 권한을 주려면?
...
- **디렉터리에 걸면**(시험 핵심): 그 안에 새로 만드는 파일·하위 디렉터리의 **소유 그룹이 자동으로 그 디렉터리의 그룹**이 된다.
- 보통은 파일 생성자의 기본 그룹이 소유 그룹이 되는데, SetGID 디렉터리에서는 디렉터리의 그룹을 물려받는다.
...
- Day 3에서 "디렉터리에 w 권한이 있으면 그 안의 파일을 (소유자가 아니어도) 삭제할 수 있다"고 했다.
- `/tmp`처럼 모두가 쓰는 디렉터리에서 이건 재앙이다 — 아무나 남의 임시 파일을 지울 수 있다. Sticky bit가 이를 막는다.
...
- 표준 권한의 근본 한계: 한 파일에 대해 **단 하나의 소유자, 단 하나의 그룹**만 지정할 수 있다.
- "alice에게는 rw, bob에게는 r, dev 그룹에게는 rw, 그 외엔 차단"처럼 세분화된 요구는 3칸으로 표현 불가능하다.
- **ACL**(Access Control List)은 파일마다 "사용자별·그룹별 권한 항목 목록"을 추가로 붙여 이 한계를 푼다.
...
- **특수 권한** : 9비트 앞에 붙는 네 번째 8진수 자리. SetUID(4)·SetGID(2)·Sticky(1).
- **SetUID** : 실행하는 사람이 누구든 **파일 소유자의 권한**으로 돌아가게 하는 비트. owner의 x 자리에 `s`.
- **SetGID(파일)** : 실행 시 소유 그룹 권한으로 동작하게 하는 비트.
- **SetGID(디렉터리)** : 그 안에 만든 파일의 그룹을 디렉터리 그룹으로 **상속**시키는 비트. 협업 디렉터리의 핵심.
- **Sticky bit** : 공유 디렉터리에서 **자기 파일만 지울 수 있게** 제한하는 비트. `/tmp`가 1777인 이유.
- **대문자 `S`/`T`** : 특수 권한은 켜졌는데 그 자리의 실행 권한이 없다는 경고 표시.
- **`find / -perm -4000`** : 시스템의 SetUID 파일을 훑는 보안 감사 표준 명령.
- **권한 상승(privilege escalation)** : SetUID 프로그램의 취약점을 타고 root 권한을 탈취하는 공격.
- **심링크 공격** : 공유 디렉터리의 파일을 심볼릭 링크로 바꿔치기하는 공격. Sticky bit가 막는다.
- **ACL** : 파일마다 사용자·그룹별 개별 권한 항목을 덧붙이는 확장 권한 목록.
- **`getfacl` / `setfacl`** : ACL을 조회하는 명령과 설정·삭제하는 명령(`-m` 추가, `-x` 항목 삭제, `-b` 전체 삭제).
- **mask** : 이름이 붙은 ACL 항목과 소유 그룹이 가질 수 있는 권한의 **상한선**. 이것 때문에 준 권한이 안 먹기도 한다.
- **`ls -l`의 `+`** : 그 파일에 ACL이 걸려 있다는 표시.
- **기본(default) ACL** : `setfacl -m d:...`로 디렉터리에 걸어, 새로 만드는 하위 항목에 상속시키는 ACL.
