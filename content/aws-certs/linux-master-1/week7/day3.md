...
핵심 통찰: 리눅스 커널은 **모듈식 단일 커널(modular monolithic kernel)**이다. 핵심 기능은 한 덩어리(monolithic)로 메모리에 상주하지만, 장치 드라이버나 파일 시스템 같은 기능은 **필요할 때 끼우고 뺄 수 있는 모듈**로 분리돼 있다. 새 USB 장치를 꽂으면 해당 드라이버 모듈이 자동 적재되고, 안 쓰면 내릴 수 있다. 덕분에 커널 전체를 다시 컴파일하지 않고도 기능을 동적으로 확장한다.
...
리눅스 커널은 부팅 시 메모리로 올라오는 압축된 단일 이미지 파일이며, 보통 `/boot` 아래에 있다.
...
모듈(커널 객체, `.ko` = kernel object)은 커널에 동적으로 끼우는 부품이다. 현재 적재된 모듈과 각 모듈의 정보를 보는 것이 첫걸음이다.
...
모듈을 올리고 내리는 명령은 두 갈래다. **저수준**(`insmod`/`rmmod`)은 단일 모듈을 직접 다루고, **고수준**(`modprobe`)은 의존성까지 자동 처리한다. 이 구조는 어제 배운 rpm vs yum 관계와 똑같다.
...
새 하드웨어 지원, 기능 최적화, 패치 적용 등을 위해 커널 소스를 직접 컴파일하기도 한다. 어제 배운 소스 컴파일과 닮았지만, **설정(config) 단계**가 핵심이라는 점이 다르다. 흐름은 다음과 같다.
...
커널은 하드웨어를 통제하는 운영체제의 심장이며, 리눅스는 핵심을 한 덩어리로 두되 기능을 동적 모듈로 분리한 **모듈식 모놀리식** 구조다. `uname -r`로 버전을, `/lib/modules/버전/`에서 모듈을, `/boot/vmlinuz-*`에서 커널 이미지를 본다. 모듈 관리는 조회(`lsmod`, `modinfo`), 적재(`insmod` 저수준 / `modprobe` 고수준·의존성 자동), 제거(`rmmod` / `modprobe -r`)로 나뉜다 — rpm vs yum과 같은 구도다. 커널 컴파일은 `make menuconfig`(설정·`.config`)→`make`(빌드)→`make modules_install`(모듈을 /lib/modules)→`make install`(커널을 /boot + GRUB) 순서이며, 설정 시 `*`(내장)/`M`(모듈)/빈칸(제외) 선택이 핵심이다.
