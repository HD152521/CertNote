...
- 가상화의 유일한 분기점은 **커널을 새로 띄우느냐(VM), 빌려 쓰느냐(컨테이너)**다. VM은 무겁고(수 GB·수십 초) 다른 OS도 돌리며, 컨테이너는 가볍고(수십 MB·1초) **리눅스 커널 위에서만** 돈다.
- **전가상화**는 게스트 커널 수정이 **불필요**(하이퍼바이저가 가로챔, KVM·ESXi), **반가상화**는 커널을 고쳐 **하이퍼콜**로 요청한다(Xen PV).
- **Type 1**은 하드웨어 바로 위(Xen·ESXi·Hyper-V·**KVM**), **Type 2**는 호스트 OS 위의 응용 프로그램(VirtualBox·VMware Workstation).
- KVM은 **CPU·메모리**를 하드웨어 가속하고 **QEMU**가 주변 장치를 흉내 내며 **libvirt/virsh**가 관리한다. **`destroy`(강제 종료) ≠ `undefine`(정의 삭제).**
- 컨테이너는 **namespace(시야 격리) + cgroup(자원 제한)**만으로 성립한다. Docker에서는 **`rm`(컨테이너)/`rmi`(이미지)**, **`-p 호스트:컨테이너`**, Dockerfile의 **`RUN`(빌드 시점)/`CMD`(실행 시점)** 구분이 핵심.

## 전가상화와 반가상화

- 게스트가 하드웨어에 접근하려 할 때 **그 요청을 누가 어떻게 받느냐**로 방식이 갈린다.
...
- **하이퍼바이저(VMM)**는 가상 머신을 만들고 물리 자원을 나눠주는 계층이다.
- 분류 기준은 하나 — **하드웨어 바로 위인가, 호스트 OS 위인가.**
...
- Type 2가 느린 이유: 게스트의 요청이 **하이퍼바이저 → 호스트 OS → 하드웨어**로 한 단계를 더 거친다.
- 그래서 서버실은 Type 1, 내 노트북은 Type 2가 정석이다.
...
- **KVM(Kernel-based Virtual Machine)**은 커널 2.6.20부터 메인라인에 포함된 리눅스 표준 가상화다.
- 핵심은 **커널 모듈**이라는 점 — 어제 배운 모듈 관리 명령이 그대로 적용된다.
...
- `virsh`는 실기 단답의 핵심이다.
- 용어 주의: libvirt에서는 가상 머신 하나를 **도메인(domain)**이라 부른다.
...
- 새 VM은 **`virt-install`**로, 디스크 이미지는 **`qemu-img`**로 만든다.
- **`raw`** : 단순해 빠르지만 처음부터 전체 용량을 차지한다.
- **`qcow2`** : 실제 쓴 만큼만 커지는 **씬 프로비저닝**과 **스냅샷**을 지원해 기본 선택지다.
...
- 컨테이너는 **게스트 OS를 띄우지 않는다.**
- 호스트 커널을 그대로 쓰면서 **커널의 두 기능**으로 프로세스를 가둔다.
...
- 두 기능을 한 줄로: **"시야와 한도"**.
  - **namespace는 시야** — 컨테이너 안에서 `ps`를 치면 자기 프로세스만, `ip addr`을 치면 자기 인터페이스만 보인다.
  - **cgroup은 한도** — 한 컨테이너가 호스트 자원을 통째로 잡아먹지 못하게 막는다.
...
- 컨테이너가 삭제되면 **쓰기 계층이 통째로 사라진다.** 그래서 보존할 데이터는 **볼륨**으로 밖에 둔다.
- `-v /host/data:/var/lib/mysql`처럼 호스트 디렉터리를 연결하면(**바인드 마운트**) 컨테이너를 지워도 데이터가 남는다.

### Dockerfile — 이미지를 코드로 정의하기

- Dockerfile은 **이미지를 만드는 절차를 적은 텍스트 파일**이며, 각 지시어가 이미지의 한 **계층(layer)**이 된다.
...
- **Podman**은 RHEL 8부터 Docker를 대체하는 기본 컨테이너 엔진이다.
- Docker와의 차이는 두 가지 — **데몬리스**(상주 데몬 dockerd 없이 컨테이너를 직접 띄움), **루트리스**(일반 사용자 권한으로 실행).
- 옵션은 Docker 호환이라 `podman run -d -p 8080:80 nginx`가 그대로 통한다. 시스템 컨테이너 계열 **LXC/LXD**도 알아둔다.
...
- **VirtualBox**는 오라클이 배포하는 대표적 **Type 2** 하이퍼바이저다.
- GUI 외에 **`VBoxManage`** 명령행 도구를 제공하며, `VBoxManage <하위명령> <VM이름> [옵션]` 구조로 일관된다.
...
- 함께 설치하는 **Guest Additions**는 게스트 안에 넣는 보조 드라이버다. 해상도 자동 조절·클립보드 공유·공유 폴더를 가능하게 한다(VMware Tools와 같은 역할).

## 암기 팁

- 가상화 문제는 **"같은 일을 하는 다른 도구의 명령"**을 짝짓거나 비슷한 이름을 뒤바꾼 보기로 나온다.
- 세 도구를 같은 작업끼리 묶어 외운다.
  - **전체 목록** : `virsh list --all` / `docker ps -a` / `VBoxManage list vms`
  - **강제 종료** : `virsh destroy` / `docker kill` / `controlvm poweroff`
  - **삭제** : `virsh undefine` / `docker rm` / `unregistervm`
...
- **가상화** : 물리 자원을 논리적으로 쪼개 한 대의 서버가 여러 대인 것처럼 동작하게 하는 기술.
- **하이퍼바이저(VMM)** : 가상 머신을 만들고 물리 자원을 나눠주는 계층. 위치에 따라 Type 1/Type 2로 나뉜다.
- **전가상화** : 게스트를 "속여" 커널 수정 없이 그대로 돌리는 방식. 하이퍼바이저가 특권 명령을 가로챈다.
- **반가상화** : 게스트 커널을 고쳐 **하이퍼콜**로 직접 요청하게 만드는 방식. 빠르지만 커널 수정이 필요하다.
- **VT-x / AMD-V** : CPU가 제공하는 하드웨어 가상화 지원. `/proc/cpuinfo`의 `vmx`/`svm` 플래그로 확인한다.
- **KVM** : 리눅스 커널 자체를 하이퍼바이저로 바꾸는 커널 모듈. CPU·메모리 가상화를 담당한다.
- **QEMU** : 디스크·NIC·VGA 등 주변 장치를 흉내 내는 사용자 공간 에뮬레이터. KVM과 분업한다.
- **libvirt / virsh** : 가상화 관리를 추상화한 계층과 그 명령행 클라이언트. Xen·LXC에도 같은 명령이 통한다.
- **도메인(domain)** : libvirt가 가상 머신 하나를 부르는 이름.
- **`virsh destroy` vs `undefine`** : 전원 코드를 뽑듯 강제로 끄기(다시 켤 수 있음) vs 도메인 정의 자체를 지우기.
- **qcow2** : 실제 쓴 만큼만 커지고 스냅샷을 지원하는 QEMU 디스크 이미지 형식. raw는 처음부터 전체 용량을 잡는다.
- **namespace** : 컨테이너가 **무엇을 볼 수 있는지**를 격리하는 커널 기능(PID·네트워크·마운트 등).
- **cgroup** : 컨테이너가 **자원을 얼마나 쓸 수 있는지**를 제한하는 커널 기능(CPU·메모리·I/O).
- **이미지 vs 컨테이너** : 실행되지 않는 읽기 전용 템플릿 vs 그 위에 쓰기 계층을 얹은 실행 인스턴스.
- **`docker rm` vs `rmi`** : 컨테이너 삭제 vs 이미지 삭제. 끝의 `i`가 image다.
- **`RUN` vs `CMD`** : 빌드 시점에 실행되어 이미지에 굳는 지시어 vs 컨테이너를 띄울 때 실행되는 기본 명령.
- **Podman** : 데몬 없이(데몬리스), 일반 사용자 권한으로(루트리스) 도는 Docker 호환 컨테이너 엔진.
