...
`crontab`은 정기 반복 작업을 예약한다. 어제 파일 구조를 봤다면 오늘은 명령 사용법이다.
...
현대 리눅스는 **systemd**가 서비스(데몬)를 관리한다. `systemctl`이 핵심 명령이다.
...
서비스 정의는 **유닛 파일**(`.service`)에 담긴다. 위치와 핵심 섹션을 알아두자.
...
프로세스·스케줄링·systemd 작업형은 "지금 동작(start/nice/fg)"과 "영구 설정(enable/crontab/set-default)"을 구분하는 게 핵심이다. 시나리오에 "부팅 후에도", "정기적으로"가 들어가면 영구 설정 명령을 골라야 한다. 다음 날은 오늘까지 배운 시스템 작업형 전체를 12문항 모의고사와 오답노트로 점검한다.
