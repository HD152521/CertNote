...
- 이번 주 전 범위(명령어 옵션 · 설정 파일 · 디스크/LVM/권한 · 프로세스/스케줄링/systemd)를 **12문항 모의고사**로 점검한다.
- 작업형은 "이 작업에 어떤 **명령 / 옵션 / 경로**인가"를 즉답할 수 있느냐의 싸움이다.
- 정답만 보지 말고 **왜 다른 보기가 틀렸는지** 오답 해설까지 읽어 헷갈리는 지점을 메운다.
- 문제의 절반은 **"지금 동작" vs "영구 설정"**을 가르는 문제다 — `start`/`mount`/`at` 대 `enable`/`fstab`/`crontab`.
- 틀린 문제는 반드시 해당 Day 본문으로 돌아가 표를 다시 외운다.
...
- **작업형** : 실기 2차에서 명령·옵션·경로를 직접 적어 내는 유형. 개념 설명이 아니라 "무엇을 치는가"를 묻는다.
- **8진수 권한** : rwx를 숫자로 바꾼 표기. r=4, w=2, x=1이라 rwx=7, rw-=6, r-x=5, r--=4가 된다.
- **재귀 옵션** : 하위 디렉터리까지 한 번에 적용하는 옵션. chmod·chown은 대문자 `-R`이다.
- **SIGHUP(1)** : 데몬에게 "설정 파일을 다시 읽어라"고 알리는 시그널. 재시작 없이 reload할 때 쓴다.
- **umask** : 새로 만들어지는 파일·디렉터리의 기본 권한에서 빼낼 비트. 파일은 666−umask, 디렉터리는 777−umask.
- **resize2fs** : LV를 늘린 뒤 ext2/3/4 파일시스템의 실제 크기까지 맞춰 주는 명령. xfs는 `xfs_growfs`.
- **vgextend** : 볼륨 그룹에 새 물리 볼륨(PV)을 추가해 저장 풀을 키우는 LVM 명령.
- **`enable --now`** : 부팅 자동 시작 등록(enable)과 즉시 시작(start)을 한 번에 하는 systemctl 옵션.
- **daemon-reload** : systemd 유닛 파일을 고친 뒤 변경 내용을 systemd에 반영시키는 명령. 빠뜨리면 수정이 무시된다.
