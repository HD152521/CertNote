...
- 실기 두 번째 난관은 **설정 파일 빈칸 채우기**다. 경로 하나, 필드 순서 하나, 지시어 철자 하나가 틀리면 점수가 날아간다.
- 계정 3대 파일의 필드 수: **passwd 7 / shadow 9 / group 4**. shadow의 4번째는 최소 변경 간격, **5번째가 최대 사용 일수**다.
- `/etc/fstab`은 **6개 필드**. 5번째는 dump 백업 여부, **6번째는 fsck 검사 순서**(루트=1, 나머지=2, 검사 안 함=0).
- crontab 시간 필드는 **분-시-일-월-요일**. `/etc/crontab`에만 실행 사용자 필드가 추가로 붙는다.
- 커널 파라미터는 `/etc/sysctl.conf`에 적고 **`sysctl -p`**로 적용. 새 계정 암호 정책 기본값은 `/etc/login.defs`의 `PASS_*` 지시어.
- 네트워크: 이름↔IP 정적 매핑은 `/etc/hosts`, DNS 서버 지정은 `/etc/resolv.conf`의 `nameserver`.

## 계정 관리 3대 파일: passwd / shadow / group

- 리눅스 사용자 계정 정보는 세 파일에 나뉘어 저장된다.
- 경로와 필드 구조를 통째로 외워야 한다.
...
- `/etc/fstab`은 부팅 시 자동으로 마운트할 파일시스템을 정의한다.
- **6개 필드** 순서가 단골 출제다.
...
- `/etc/sysctl.conf`는 부팅 시 적용할 커널 파라미터를 정의한다.
- `/proc/sys/` 아래 값과 1:1 대응한다.
...
직접 `cat`으로 열어 실제 내용을 보면서 필드 위치를 손에 익혀라. 다음 날은 이 지식을 바탕으로 디스크·LVM·권한을 실제로 수행하는 작업형을 다룬다.

## 📖 용어

- **/etc/passwd** : 사용자 계정의 기본 정보를 담은 7필드 파일. 2번째 `x`는 "실제 암호는 shadow에 있다"는 표시다.
- **UID / GID** : 사용자 번호 / 그룹 번호. UID 0은 언제나 root다.
- **GECOS** : `/etc/passwd`의 5번째 필드. 이름·전화 같은 사람이 읽는 설명이 들어간다.
- **/etc/shadow** : 암호화된 비밀번호와 만료 정책을 담은 9필드 파일. 일반 사용자는 읽을 수 없다.
- **/sbin/nologin** : 로그인 셸 자리에 넣어 그 계정의 로그인을 막는 특수 셸. `/bin/false`도 같은 용도다.
- **/etc/fstab** : 부팅 시 자동 마운트할 파일시스템을 정의하는 6필드 파일.
- **dump 필드** : fstab 5번째 필드. 백업 대상 여부를 0/1로 표시한다.
- **fsck 순서** : fstab 6번째 필드. 부팅 시 검사 순서로, 루트는 1, 나머지는 2, 검사 제외는 0.
- **crontab 시간 필드** : 분-시-일-월-요일 순의 5개 필드. `*/N`은 N마다, 숫자만 쓰면 정시 실행이다.
- **sysctl -p** : `/etc/sysctl.conf`를 다시 읽어 커널 파라미터를 적용하는 명령.
- **PASS_MAX_DAYS** : `/etc/login.defs`에서 새 계정의 암호 최대 사용 일수를 정하는 지시어. 기존 계정은 `chage`로 바꾼다.
- **/etc/resolv.conf** : 사용할 DNS 서버를 `nameserver` 지시어로 적는 파일.
- **/etc/nsswitch.conf** : 이름을 파일(`files`)로 먼저 볼지 DNS로 먼저 볼지 조회 순서를 정하는 파일.
