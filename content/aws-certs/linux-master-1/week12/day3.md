...
- 작업형은 **순서가 틀리면 전체가 무너진다**. 명령 하나가 아니라 시퀀스를 통째로 외운다.
- 디스크 추가 4단계: **파티션(fdisk) → 포맷(mkfs) → 마운트(mount) → fstab 등록**. "부팅 후에도 유지"면 fstab이 필수다.
- LVM은 **pvcreate → vgcreate → lvcreate** 순으로 쌓고, 지울 때는 반대 순서다.
- 용량 확장은 **vgextend(풀 키우기) → lvextend(LV 키우기) → resize2fs/xfs_growfs(파일시스템 키우기)**. 마지막 단계를 빼먹으면 용량이 안 늘어난다.
- umask는 **기본값에서 빼는 마스크** — 파일은 666, 디렉터리는 777에서 뺀다. `022` → 644 / 755.
- 특수 권한: **SetUID 4000**(소유자 권한 실행), **SetGID 2000**(그룹 상속), **Sticky 1000**(본인 파일만 삭제). `/tmp`=1777, `passwd`=4755.
...
- LVM은 디스크를 유연하게 묶고 나누는 계층 구조다.
- **물리 볼륨(PV) → 볼륨 그룹(VG) → 논리 볼륨(LV)** 순서로 쌓는다.
...
각 단계의 명령 접두어(pv/vg/lv)와 마무리 단계(fstab, resize2fs)를 절대 빠뜨리지 마라. 다음 날은 프로세스·스케줄링·systemd 작업형으로 시스템 운영의 나머지 절반을 채운다.

## 📖 용어

- **fdisk** : 디스크를 파티션으로 나누는 대화형 도구. `n`(생성) → `w`(저장) 순으로 쓴다.
- **mkfs** : 파티션에 파일시스템을 만드는(포맷) 명령. 디스크 전체가 아니라 **파티션**에 건다.
- **fstab 등록** : 부팅 시 자동 마운트되게 만드는 마지막 단계. 이걸 빼면 재부팅 때 마운트가 사라진다.
- **mount -a** : `/etc/fstab`의 모든 항목을 다시 마운트. fstab을 고친 뒤 문법 검증용으로 쓴다.
- **blkid** : 장치의 UUID와 파일시스템 타입을 알려주는 명령. 장치명 대신 UUID로 fstab을 쓸 때 필요하다.
- **PV(물리 볼륨)** : LVM이 쓸 수 있게 초기화한 디스크·파티션. `pvcreate`로 만든다.
- **VG(볼륨 그룹)** : PV 여럿을 묶은 저장 풀. `vgcreate`로 만들고 `vgextend`로 키운다.
- **LV(논리 볼륨)** : VG에서 잘라낸 논리 디스크. `lvcreate`로 만들고 `lvextend`로 키운다.
- **resize2fs / xfs_growfs** : LV를 늘린 뒤 그 위의 파일시스템까지 늘려 주는 명령(ext 계열 / xfs).
- **umask** : 새로 만드는 파일·디렉터리의 기본 권한에서 빼낼 비트. 파일은 666, 디렉터리는 777이 시작값이다.
- **SetUID** : 실행할 때 실행자가 아닌 **파일 소유자**의 권한으로 동작하게 하는 비트(4000). `passwd` 명령이 대표 예다.
- **SetGID** : 실행 시 그룹 권한을 쓰거나, 디렉터리에 걸면 새 파일이 그 디렉터리의 **그룹을 상속**하게 하는 비트(2000).
- **Sticky bit** : 디렉터리에 걸어 **자기 파일만 지울 수 있게** 하는 비트(1000). `/tmp`가 대표 예다.
