...
리눅스마스터 1급 2차 실기 시험은 "이 작업을 하려면 어떤 명령과 옵션을 써야 하는가"를 묻는다. 객관식이 아니라 **빈칸에 명령어·옵션·경로를 직접 적는 단답형**이 대부분이다. 그래서 1급 실기는 "이해"만으로는 부족하고 **손가락이 기억하는 수준의 정밀 암기**가 필요하다. `chmod 755`의 755가 무엇을 의미하는지 헷갈리거나, `find`의 `-name`과 `-iname` 차이를 놓치거나, `kill -9`와 `kill -15`를 바꿔 쓰면 그대로 0점이다. 오늘은 실기에서 가장 자주 출제되는 4대 영역 — **권한(chmod/chown), 검색(find/grep), 프로세스(ps/kill), 파일(ls/ln)** — 의 명령어 옵션을 단답 암기 카드 형태로 정리한다. 각 명령마다 "이 작업 → 이 옵션"을 즉시 떠올릴 수 있도록 반복 훈련하는 것이 목표다.

## 권한 명령어: chmod / chown / chgrp

권한은 실기 단골 중의 단골이다. `chmod`는 권한을, `chown`은 소유자를, `chgrp`는 그룹을 바꾼다.
...
8진수는 `r=4, w=2, x=1`을 더해서 만든다. `rwx = 4+2+1 = 7`, `rw- = 4+2 = 6`, `r-x = 4+1 = 5`, `r-- = 4`. 세 자리는 순서대로 **소유자(u) / 그룹(g) / 기타(o)**를 뜻한다.
...
`find`는 **파일을 조건으로 찾고**, `grep`은 **파일 내용에서 패턴을 찾는다**. 둘의 역할 구분이 핵심이다.
...
`-mtime`의 부호가 핵심이다. `+7`은 "7일 **이전**(더 오래된)", `-7`은 "7일 **이내**(더 최근)", `7`은 정확히 7일 전이다.
...
`ps aux`와 `ps -ef`는 거의 같은 정보를 다른 형식으로 보여준다. `aux`는 CPU·메모리 사용률(%CPU, %MEM)을, `-ef`는 PPID(부모 프로세스 ID)와 시작 시각을 강조한다.
...
`ls -l`의 첫 글자는 파일 종류다: `-`(일반), `d`(디렉터리), `l`(심볼릭 링크), `b`(블록 장치), `c`(문자 장치), `p`(파이프), `s`(소켓).
...
오늘 정리한 명령어 옵션은 2차 실기 시스템 작업형의 절반 이상을 차지한다. 표를 그냥 읽지 말고 직접 터미널에 쳐보며 출력 결과까지 눈에 익혀야 단답형에서 막힘없이 적을 수 있다. 다음 날은 설정 파일 경로와 핵심 지시어를 빈칸 채우기 형태로 굳힌다.
