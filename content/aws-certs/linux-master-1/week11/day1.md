...
회사에는 윈도 PC와 리눅스 서버가 섞여 있다. 윈도 사용자는 탐색기에서 `\\서버\공유폴더`로 네트워크 드라이브를 잡고 싶어 하고, 리눅스 서버는 그 파일을 호스팅해야 한다. 문제는 두 운영체제가 서로 다른 파일 공유 프로토콜을 쓴다는 점이다. 윈도는 **SMB/CIFS**(Server Message Block), 리눅스는 전통적으로 **NFS**를 쓴다. 이 간극을 메우는 도구가 바로 **Samba**다. Samba는 리눅스/유닉스 시스템 위에서 SMB 프로토콜을 구현해, 리눅스 서버가 마치 윈도 파일 서버처럼 보이게 만든다. 오늘은 Samba의 구성 요소, 핵심 설정 파일인 `smb.conf`, 공유 정의 방법, 그리고 Samba 전용 계정을 만드는 `smbpasswd`까지 실기 시험에 자주 나오는 부분을 정밀하게 짚는다.
...
`smbd`가 실제 파일 전송과 인증을 담당하는 핵심이고, `nmbd`는 윈도 네트워크 환경에서 "내 네트워크 환경"에 서버 이름이 보이게 해주는 이름 해석 서비스다. 현대 환경에서는 DNS와 445 포트만으로도 동작하지만, 시험에서는 두 데몬의 역할 구분이 단골이다.
...
Samba의 동작은 거의 전부 `/etc/samba/smb.conf` 하나로 결정된다. 이 파일은 윈도의 INI 파일과 비슷한 구조로, **섹션(section)**으로 나뉜다. 섹션 이름은 대괄호 `[ ]`로 감싼다.
...
`[global]`을 제외한 나머지 섹션 이름은 곧 **공유 이름(share name)**이 된다. 윈도에서 `\\LINUXSRV\public`로 접근하면 `[public]` 섹션의 설정이 적용된다.
...
`workgroup`은 윈도 작업 그룹 이름을, `server string`은 네트워크에서 보이는 서버 설명을 지정한다. `%v`(버전), `%m`(클라이언트 NetBIOS 이름) 같은 **치환 변수**를 쓸 수 있다.
...
`smb.conf`를 수정한 뒤 오타를 잡으려면 **`testparm`**으로 문법을 검사한다. 실무에서 서비스를 재시작하기 전에 반드시 거치는 단계다.
...
출력에 `Loaded services file OK.`가 나오면 문법은 정상이다. 이어서 Enter를 누르면 실제로 적용될 모든 설정값을 펼쳐 보여준다.

> 💡 **실무 흐름**: `smb.conf 수정 → testparm으로 검증 → systemctl restart smb → smbclient로 접속 테스트`. 이 순서를 몸에 익혀라.

## smbpasswd: Samba 전용 계정 만들기

`security = user`에서는 접속자가 **Samba 비밀번호**를 가져야 한다. 핵심은 이것이 리눅스 로그인 비밀번호(`/etc/shadow`)와 **별개**라는 점이다. Samba는 자체 암호 데이터베이스(기본적으로 TDB 형식, 과거엔 `/etc/samba/smbpasswd` 파일)를 사용한다.

**전제 조건**: Samba 사용자는 먼저 리눅스 시스템 계정으로 존재해야 한다. Samba 계정은 기존 리눅스 사용자 위에 비밀번호만 추가하는 형태다.
...
서버 설정이 끝나면 클라이언트 쪽에서 접속을 확인해야 한다. **`smbclient`**는 FTP와 비슷한 대화형 인터페이스로 SMB 공유에 접속하는 도구다.
...
Samba는 윈도의 SMB/CIFS 프로토콜을 리눅스에서 구현해, 이종 OS 간 파일·프린터 공유를 가능하게 한다. 핵심은 **두 데몬(smbd=공유·인증, nmbd=이름 해석)**, **설정 파일 `smb.conf`의 섹션 구조(`[global]`/`[homes]`/`[printers]` + 사용자 공유 섹션)**, 그리고 **`smbpasswd`로 만드는 별도의 Samba 계정**이다. 실기에서는 `path`·`writable`·`valid users`·`guest ok` 같은 지시어의 의미, `testparm`으로 검증하는 흐름, 그리고 "리눅스 권한과 Samba 권한이 동시에 통과해야 한다"는 이중 관문 원리를 정확히 기억하면 된다. 내일은 리눅스끼리의 표준 공유인 NFS를 다루며, Samba와 어떻게 다른지 비교한다.
