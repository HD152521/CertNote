...
export function TutorPanel({ questionId, selected }: TutorPanelProps) {
  ...
  async function ask(history: Turn[]) {
    ...
    try {
      const res = await fetch('/api/tutor', {
        ...
        body: JSON.stringify({ questionId, selected, history }),
      });
      if (!res.ok || !res.body) {
  ...
  if (needPro) {
    return (
      <div className="mt-3 rounded-lg border border-accent/40 bg-accent/5 p-4 text-sm">
        <p className="flex items-center gap-1.5 font-medium text-fg"><Sparkles className="h-4 w-4 text-accent" /> AI 오답 튜터는 Pro 전용이에요</p>
        <p className="mt-1 text-fg-muted">틀린 문제를 AI가 한국어로 풀어 설명하고, 추가 질문에도 답해드려요.</p>
        <Link href="/pricing" className="mt-2 inline-block rounded-md border border-accent/40 px-3 py-1.5 text-sm font-medium text-accent transition hover:bg-accent/10">Pro 업그레이드 보기</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 rounded-lg border border-border bg-bg-elevated p-4">
      <p className="mb-2 flex items-center gap-1.5 text-sm font-medium text-fg"><Sparkles className="h-4 w-4 text-accent" /> AI 설명</p>
      <div ref={scrollRef} className="max-h-80 space-y-3 overflow-y-auto">
        ...
        {busy && !streaming && <p className="text-sm text-fg-faint">생각 중…</p>}
      </div>
