...
describe('hreflang 상호 참조 — 최상위 3개(/, /aws-certs, /en)', () => {
  test('홈(/)은 hreflang 언어 쌍을 선언하지 않는다(영어판 URL이 따로 없음)', () => {
    expect(homeMetadata.alternates?.canonical).toBe('/');
    expect(homeMetadata.alternates?.languages).toBeUndefined();
  });

  test('/aws-certs ↔ /en 이 완전히 동일한 languages 객체로 상호 참조한다', async () => {
    const hub = await generateCategoryMetadata({ params: Promise.resolve({ category: 'aws-certs' }) });

    expect(hub.alternates?.canonical).toBe('/aws-certs');
    ...
    expect(hub.alternates?.languages).toEqual({ ko: '/aws-certs', en: '/en', 'x-default': '/aws-certs' });
  });

  test('x-default는 사이트 전체에서 정확히 하나의 URL(/aws-certs)만 가리킨다', async () => {
    const hub = await generateCategoryMetadata({ params: Promise.resolve({ category: 'aws-certs' }) });
    expect(hub.alternates?.languages).toMatchObject({ 'x-default': '/aws-certs' });
    expect(enHomeMetadata.alternates?.languages).toMatchObject({ 'x-default': '/aws-certs' });
    // 홈은 hreflang 자체가 없으므로 x-default도 선언하지 않는다 — 중복 선언 방지.
    expect(homeMetadata.alternates?.languages).toBeUndefined();
  });
});

describe('hreflang 상호 참조 — 자격증 허브(/aws-certs/saa-c03 ↔ /en/saa-c03)', () => {
  test('양쪽 페이지의 languages가 완전히 동일하다(회귀 확인)', async () => {
    const ko = await generateCertMetadata({ params: Promise.resolve({ category: 'aws-certs', slug: 'saa-c03' }) });
    const en = await generateCertMetadata({ params: Promise.resolve({ category: 'en', slug: 'saa-c03' }) });

    expect(ko.alternates?.canonical).toBe('/aws-certs/saa-c03');
    expect(en.alternates?.canonical).toBe('/en/saa-c03');
    expect(ko.alternates?.languages).toEqual(en.alternates?.languages);
    expect(ko.alternates?.languages).toEqual({ ko: '/aws-certs/saa-c03', en: '/en/saa-c03' });
  });

  test('영어판이 없는 자격증(linux-master-1)은 깨진 hreflang을 만들지 않는다', async () => {
    const meta = await generateCertMetadata({ params: Promise.resolve({ category: 'aws-certs', slug: 'linux-master-1' }) });
    expect(meta.alternates?.canonical).toBe('/aws-certs/linux-master-1');
    expect(meta.alternates?.languages).toBeUndefined();
  });
});

describe('hreflang 상호 참조 — day 페이지(week1, 무료)', () => {
  test('/aws-certs/saa-c03/week1/day1 ↔ /en/saa-c03/week1/day1 languages가 완전히 동일하다', async () => {
    const ko = await generateDayMetadata({
      params: Promise.resolve({ category: 'aws-certs', slug: 'saa-c03', week: 'week1', day: 'day1' }),
    });
  ...
  test('linux-master-1(영어판 없음)의 무료 주차는 hreflang을 선언하지 않는다', async () => {
    const meta = await generateDayMetadata({
      params: Promise.resolve({ category: 'aws-certs', slug: 'linux-master-1', week: 'week1', day: 'day1' }),
    });
    expect(meta.alternates?.languages).toBeUndefined();
  });
});
