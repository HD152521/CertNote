...
function relTime(iso: string, lang: Language): string {
  ...
  if (day < 30) return lang === 'en' ? `${day}d ago` : `${day}일 전`;
  return lang === 'en' ? `${Math.floor(day / 30)}mo ago` : `${Math.floor(day / 30)}개월 전`;
}
...
function CertRow({ c, lang }: { c: CertProgress; lang: Language }) {
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <span className="shrink-0 text-xs text-fg-muted tabular-nums">
          {acc === null ? (lang === 'en' ? 'Not started' : '미시작') : (lang === 'en' ? `${acc}% accuracy` : `정답률 ${acc}%`)}
        </span>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 py-10">
      <header className="space-y-1">
        <p className="font-mono text-xs uppercase tracking-wider text-fg-faint">{lang === 'en' ? 'Dashboard' : 'Dashboard'}</p>
        <h1 className="text-2xl font-semibold tracking-tight">{lang === 'en' ? 'Learning Dashboard' : '학습 대시보드'}</h1>
        <p className="text-sm text-fg-muted">{lang === 'en' ? `${user.email}'s learning status` : `${user.email}님의 학습 현황`}</p>
      </header>
      ...
      {!hasActivity ? (
        <div className="rounded-xl border border-border bg-bg-elevated p-8 text-center">
          ...
          <p className="mb-1 font-medium text-fg">{lang === 'en' ? 'No practice records yet' : '아직 풀이 기록이 없어요'}</p>
          <p className="mb-4 text-sm text-fg-muted">
            {lang === 'en'
              ? 'Choose a certification and solve practice problems to see your progress, accuracy, and review status here.'
              : '자격증을 골라 연습 문제를 풀면 여기에 진도·정답률·복습 현황이 쌓입니다.'}
          </p>
          <Link href="/" className="text-sm text-accent underline underline-offset-4">
            {lang === 'en' ? 'Start Learning' : '학습 시작하기'}
          </Link>
        </div>
      ) : (
        <>
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="col-span-2 row-span-1 rounded-xl border border-accent/30 bg-accent/5 px-5 py-5 sm:col-span-2">
              <p className="text-xs text-fg-muted">{lang === 'en' ? 'Overall Accuracy' : '전체 정답률'}</p>
              ...
              <p className="mt-1 text-xs text-fg-faint tabular-nums">
                {lang === 'en' ? `${data.correct} / ${data.attempts} correct` : `${data.correct} / ${data.attempts}문제 정답`}
              </p>
            </div>
            <StatTile label={lang === 'en' ? 'Questions Solved' : '푼 문제'} value={data.attemptedQuestions} suffix={`/ ${data.totalQuestions}`} />
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-fg-muted">{lang === 'en' ? 'Progress by Certification' : '자격증별 진행'}</h2>
              {data.review.due > 0 && (
                <Link href="/review" className="text-xs text-accent underline underline-offset-4">
                  {lang === 'en' ? `Start Review (${data.review.due})` : `복습 시작 (${data.review.due})`}
                </Link>
              )}
            </div>
            <div className="space-y-2">
              {data.certs.map((c) => <CertRow key={c.slug} c={c} lang={lang} />)}
            </div>
          </section>

          {/* 도메인별 약점 — 모의고사 풀이가 쌓이면 채워진다(정답률 낮은 순) */}
          <section className="space-y-3">
            <h2 className="text-sm font-medium text-fg-muted">{lang === 'en' ? 'Domain Weaknesses' : '도메인별 약점'}</h2>
            {data.domains.length === 0 ? (
              <p className="rounded-lg border border-border bg-bg-elevated px-4 py-3 text-sm text-fg-faint">
                {lang === 'en'
                  ? `Take mock exams to see your accuracy by domain. We'll show your weak areas first.`
          ...
          {data.recent.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-sm font-medium text-fg-muted">{lang === 'en' ? 'Recent Activity' : '최근 학습'}</h2>
              <ul className="space-y-1.5">
                {data.recent.map((r, i) => (
                  <li key={`${r.questionId}-${i}`}>
                    ...
                    >
                      ...
                      <span className="hidden sm:inline shrink-0 font-mono text-[10px] uppercase text-fg-faint">
                        {r.week > 0 ? `${r.slug} W${r.week}D${r.day}` : `${r.slug} ${lang === 'en' ? 'Mock Exam' : '모의고사'}`}
                      </span>
                      <span className="shrink-0 text-[11px] text-fg-faint">{relTime(r.attemptedAt, lang)}</span>
                    </Link>
                  </li>
                ))}
              </ul>
