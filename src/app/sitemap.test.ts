...
describe('sitemap', () => {
  // sitemap()은 전체 content 디렉터리를 fs로 순회한다(수 초). 전체 스위트를 병렬로 돌리면
  // 디스크 경합으로 기본 5s 타임아웃을 넘길 수 있어 sitemap()을 호출하는 테스트에 여유를 준다.
  test('/privacy가 사이트맵에 포함된다(누락 회귀 방지)', { timeout: 30000 }, async () => {
    const entries = await sitemap();
    const privacy = entries.find((e) => e.url.endsWith('/privacy'));
    expect(privacy).toBeDefined();
  });

  test('robots.txt로 차단된 라우트는 사이트맵에 없다', { timeout: 30000 }, async () => {
    const entries = await sitemap();
  ...
  test('Week2+(유료) day 페이지는 사이트맵에서 제외된다', { timeout: 30000 }, async () => {
    const entries = await sitemap();
    const paidDay = entries.find((e) => /\/week[2-9]\/day/.test(e.url) || /\/week1[0-9]\/day/.test(e.url));
    expect(paidDay).toBeUndefined();
  });

  test('무료 주차 상한(FREE_WEEK) 밖의 URL이 없다 — 정책 상수와 항상 일치', { timeout: 30000 }, async () => {
    const entries = await sitemap();
  ...
  test('같은 콘텐츠에 대해 두 번 호출해도 모든 lastModified가 동일하다(결정성)', { timeout: 30000 }, async () => {
  ...
  test('모든 lastModified가 new Date() 대신 고정된(과거) 값이다 — 매 호출마다 "지금"으로 갱신되지 않는다', { timeout: 30000 }, async () => {
  ...
  test('day 페이지의 lastModified가 매니페스트의 해당 파일 ISO 시각과 정확히 일치한다', { timeout: 30000 }, async () => {
    ...
    const entries = await sitemap();
    const day1 = entries.find((e) => e.url.endsWith('/aws/saa-c03/week1/day1'));

    expect(day1).toBeDefined();
    expect(new Date(day1?.lastModified as string | Date).getTime()).toBe(new Date(expectedIso).getTime());
  });

  test('홈의 lastModified가 매니페스트의 src/app/page.tsx ISO 시각과 정확히 일치한다(자격증 meta.json보다 최신인 경우)', { timeout: 30000 }, async () => {
    const manifestRaw = await fs.readFile(path.join(process.cwd(), 'src', 'data', 'content-manifest.json'), 'utf8');
    const manifest = JSON.parse(manifestRaw) as { files: Record<string, string> };
